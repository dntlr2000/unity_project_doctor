...
description: "Verify a Unity 6000.0.69f1 script-compilation baseline by validating an existing unity-project-doctor v0.2 JSON result, copying the exact current Unity project into an isolated system-temporary directory, running only the specified Unity.exe in batch mode against that copy, analyzing its exit code and Editor.log, and proving the original project file list and SHA-256 hashes are unchanged. Use only when the user explicitly invokes $unity-baseline-verification; never use it from implicit intent or an ordinary Unity request."
---

# Unity Baseline Verification v0.1
...
- Require an existing unity-project-doctor v0.2 JSON document for the exact current project.
...
- Reject unsafe reparse points and file package dependencies that resolve outside the original project.
...
Preserve Doctor warnings in the baseline JSON. Reject AUDIT_BLOCKED and NOT_A_UNITY_PROJECT without launching Unity.
...
- BASELINE_VERIFIED: Doctor, version, isolation, exit code, explicit import and compilation log markers, and original integrity all passed.
...
3. Detected Unity version and the exact isolated project path.
4. Unity exit code and Editor.log classification.
